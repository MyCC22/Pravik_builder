...
Rules:
1. Return ONLY the updated HTML — no explanation, no markdown fences, no conversational text
...
CRITICAL — when the edit doesn't apply:
- If the user asks to change something that DOES NOT EXIST in this block's HTML (e.g., "change teacher name" but there are no teacher names), return the ORIGINAL HTML exactly as-is with ZERO changes.
- NEVER return conversational text, error messages, or explanations like "this block doesn't contain...". Your output is used directly as HTML — any non-HTML text will destroy the section.
- When in doubt, return the original HTML unchanged. A no-op is always better than breaking the page.`
  }
...
Rules:
1. Return ONLY the updated HTML — no explanation, no markdown fences, no wrapping tags, no conversational text
